...
*
* Arguments:   - INT32 *a: pointer to output array (allocated)
...
**************************************************/
static UINT32 rej_uniform(INT32 *a,
                                UINT32 len,
                                CONST UINT8 *buf,
                                UINT32 buflen)
{
                  ...
                  UINT16 nonce)
{
  ...
  UINT8 buf[POLY_UNIFORM_NBLOCKS*STREAM128_BLOCKBYTES + 2];
...
*
* Arguments:   - poly *c: pointer to output polynomial
*              - CONST UINT8 mu[]: byte array containing seed of length CTILDEBYTES
**************************************************/
...
*
* Arguments:   - UINT8 *r: pointer to output byte array with at least
...
EFIAPI VOID polyt1_unpack(poly *r, CONST UINT8 *a) {
  ...
  for(i = 0; i < N/4; ++i) {
    r->coeffs[4*i+0] = ((a[5*i+0] >> 0) | ((UINT32)a[5*i+1] << 8)) & 0x3FF;
    r->coeffs[4*i+1] = ((a[5*i+1] >> 2) | ((UINT32)a[5*i+2] << 6)) & 0x3FF;
    r->coeffs[4*i+2] = ((a[5*i+2] >> 4) | ((UINT32)a[5*i+3] << 4)) & 0x3FF;
    r->coeffs[4*i+3] = ((a[5*i+3] >> 6) | ((UINT32)a[5*i+4] << 2)) & 0x3FF;
  }
...
EFIAPI VOID polyt0_unpack(poly *r, CONST UINT8 *a) {
  ...
  for(i = 0; i < N/8; ++i) {
    ...
    r->coeffs[8*i+0] |= (UINT32)a[13*i+1] << 8;
    r->coeffs[8*i+0] &= 0x1FFF;

    r->coeffs[8*i+1]  = a[13*i+1] >> 5;
    r->coeffs[8*i+1] |= (UINT32)a[13*i+2] << 3;
    r->coeffs[8*i+1] |= (UINT32)a[13*i+3] << 11;
    r->coeffs[8*i+1] &= 0x1FFF;

    r->coeffs[8*i+2]  = a[13*i+3] >> 2;
    r->coeffs[8*i+2] |= (UINT32)a[13*i+4] << 6;
    r->coeffs[8*i+2] &= 0x1FFF;

    r->coeffs[8*i+3]  = a[13*i+4] >> 7;
    r->coeffs[8*i+3] |= (UINT32)a[13*i+5] << 1;
    r->coeffs[8*i+3] |= (UINT32)a[13*i+6] << 9;
    r->coeffs[8*i+3] &= 0x1FFF;

    r->coeffs[8*i+4]  = a[13*i+6] >> 4;
    r->coeffs[8*i+4] |= (UINT32)a[13*i+7] << 4;
    r->coeffs[8*i+4] |= (UINT32)a[13*i+8] << 12;
    r->coeffs[8*i+4] &= 0x1FFF;

    r->coeffs[8*i+5]  = a[13*i+8] >> 1;
    r->coeffs[8*i+5] |= (UINT32)a[13*i+9] << 7;
    r->coeffs[8*i+5] &= 0x1FFF;

    r->coeffs[8*i+6]  = a[13*i+9] >> 6;
    r->coeffs[8*i+6] |= (UINT32)a[13*i+10] << 2;
    r->coeffs[8*i+6] |= (UINT32)a[13*i+11] << 10;
    r->coeffs[8*i+6] &= 0x1FFF;

    r->coeffs[8*i+7]  = a[13*i+11] >> 3;
    r->coeffs[8*i+7] |= (UINT32)a[13*i+12] << 5;
...
EFIAPI VOID polyz_unpack(poly *r, CONST UINT8 *a) {
  ...
  for(i = 0; i < N/4; ++i) {
    ...
    r->coeffs[4*i+0] |= (UINT32)a[9*i+1] << 8;
    r->coeffs[4*i+0] |= (UINT32)a[9*i+2] << 16;
    r->coeffs[4*i+0] &= 0x3FFFF;

    r->coeffs[4*i+1]  = a[9*i+2] >> 2;
    r->coeffs[4*i+1] |= (UINT32)a[9*i+3] << 6;
    r->coeffs[4*i+1] |= (UINT32)a[9*i+4] << 14;
    r->coeffs[4*i+1] &= 0x3FFFF;

    r->coeffs[4*i+2]  = a[9*i+4] >> 4;
    r->coeffs[4*i+2] |= (UINT32)a[9*i+5] << 4;
    r->coeffs[4*i+2] |= (UINT32)a[9*i+6] << 12;
    r->coeffs[4*i+2] &= 0x3FFFF;

    r->coeffs[4*i+3]  = a[9*i+6] >> 6;
    r->coeffs[4*i+3] |= (UINT32)a[9*i+7] << 2;
    r->coeffs[4*i+3] |= (UINT32)a[9*i+8] << 10;
  ...
  for(i = 0; i < N/2; ++i) {
    ...
    r->coeffs[2*i+0] |= (UINT32)a[5*i+2] << 16;
