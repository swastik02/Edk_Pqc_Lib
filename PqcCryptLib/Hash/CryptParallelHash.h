...
//
typedef struct Keccak1600_Ctx_t {
  UINT64      A[5][5];
  UINTN       block_size;  /* cached ctx->digest->block_size */
  UINTN       md_size;     /* output length, variable in XOF */
  UINTN       num;         /* used bytes in below buffer */
  UINT8       buf[KECCAK1600_WIDTH / 8 - 32];
  UINT8       pad;
} Keccak1600_Ctx;
