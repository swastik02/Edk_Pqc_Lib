...
*
* Returns the loaded 64-bit UINT32eger
**************************************************/
...
*              - UINT64 u: input 64-bit UINT32eger
**************************************************/
...
CONST UINT64 KeccakF_RoundConstants[NROUNDS] = {
...
*
* Arguments:   - UINT64 *s: pointer to Keccak state
**************************************************/
static VOID keccak_init(UINT64 s[25])
{
  UINT32 i;
  for(i=0;i<25;i++)
    s[i] = 0;
...
*
* Arguments:   - UINT64 *s: pointer to Keccak state
*              - UINT32 pos: position in current block to be absorbed
*              - UINT32 r: rate in bytes (e.g., 168 for SHAKE128)
*              - CONST UINT8 *in: pointer to input to be absorbed into s
*              - UINTN inlen: length of input in bytes
*
* Returns new position pos in current block
**************************************************/
static UINT32 keccak_absorb(UINT64 s[25],
                                  UINT32 pos,
                                  UINT32 r,
                                  CONST UINT8 *in,
                                  UINTN inlen)
...
*              - UINT32 pos: position in current block to be absorbed
*              - UINT32 r: rate in bytes (e.g., 168 for SHAKE128)
*              - UINT8 p: domain separation byte
**************************************************/
static VOID keccak_finalize(UINT64 s[25], UINT32 pos, UINT32 r, UINT8 p)
{
  s[pos/8] ^= (UINT64)p << 8*(pos%8);
  s[r/8-1] ^= 1ULL << 63;
}
...
*              - UINTN outlen: number of bytes to be squeezed (written to out)
*              - UINT64 *s: pointer to input/output Keccak state
*              - UINT32 pos: number of bytes in current block already squeezed
*              - UINT32 r: rate in bytes (e.g., 168 for SHAKE128)
*
* Returns new position pos in current block
**************************************************/
static UINT32 keccak_squeeze(UINT8 *out,
                                   UINTN outlen,
                                   UINT64 s[25],
                                   UINT32 pos,
                                   UINT32 r)
...
**************************************************/
static VOID keccak_absorb_once(UINT64 s[25],
                               UINT32 r,
                               CONST UINT8 *in,
                               UINTN inlen,
                               UINT8 p)
{
  ...
  for(i=0;i<inlen;i++)
    s[i/8] ^= (UINT64)in[i] << 8*(i%8);

  s[i/8] ^= (UINT64)p << 8*(i%8);
  s[(r-1)/8] ^= 1ULL << 63;
}
...
*
* Arguments:   - UINT8 *out: pointer to output blocks
*              - UINTN nblocks: number of blocks to be squeezed (written to out)
*              - UINT64 *s: pointer to input/output Keccak state
*              - UINT32 r: rate in bytes (e.g., 168 for SHAKE128)
**************************************************/
...
*
* Arguments:   - UINT8 *out: pointer to output blocks
*              - UINTN outlen : number of bytes to be squeezed (written to output)
...
*
* Arguments:   - UINT8 *out: pointer to output blocks
*              - UINTN nblocks: number of blocks to be squeezed (written to output)
*              - keccak_state *s: pointer to input/output Keccak state
**************************************************/
VOID 
EFIAPI
...
*
* Arguments:   - UINT8 *out: pointer to output
*              - UINTN outlen: requested output length in bytes
*              - CONST UINT8 *in: pointer to input
*              - UINTN inlen: length of input in bytes
**************************************************/
VOID 
EFIAPI
...
*
* Arguments:   - UINT8 *h: pointer to output (32 bytes)
*              - CONST UINT8 *in: pointer to input
*              - UINTN inlen: length of input in bytes
**************************************************/
VOID 
...
*
* Arguments:   - UINT8 *h: pointer to output (64 bytes)
*              - CONST UINT8 *in: pointer to input
*              - UINTN inlen: length of input in bytes
**************************************************/
VOID
