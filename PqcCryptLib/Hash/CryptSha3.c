  ...
  )
{
  if (BlockSize <= sizeof (Context->buf)) {
    SetMem(Context->A, sizeof (Context->A), 0);
  ...
  )
{
  ...
  if ((Num = Context->num) != 0) {
    ...
    if (DataSize < Rem) {
      CopyMem (Context->buf + Num, DataCopy, DataSize);
    ...
    //
    CopyMem (Context->buf + Num, DataCopy, Rem);
    DataCopy += Rem;
    DataSize -= Rem;
  ...
  if (Rem > 0) {
    CopyMem (Context->buf, DataCopy + DataSize - Rem, Rem);
    Context->num = Rem;
  }
  ...
  )
{
  ...
  // same byte.
  //
  SetMem (Context->buf + Num, BlockSize - Num, 0);
